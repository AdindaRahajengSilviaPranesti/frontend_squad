...
export class WaterComponent {
  ...
  lineChartWaterAl4: any;
  lineChartWaterNami: any;
  lineChartWaterCan: any;
  lineChartWaterPet: any;
  ...
  ngOnInit():void {
    ...
    // GET AREA SAMPLING AIR NAMI
    this.restApiServices.getAreaSamplingNami().subscribe((data) => {
      this.areaNamis = data
      console.log('area nami',this.areaNamis);
    });
    // END GET AREA SAMPLING AIR NAMI


    // DEFAULT CHART
    this.restApiServices.waterSamplingAl4(this.defaultFormSubmit).subscribe(data=>{
      let airAl4 = data;
      console.log('water al4',airAl4);
      this._lineChartWaterAl4(this.defaultFormSubmit, airAl4)
      
    })
    this.restApiServices.waterSamplingNami(this.defaultFormSubmit).subscribe(data=>{
      let airNami = data;
      console.log('water Nami',airNami);
      this._lineChartWaterNami(this.defaultFormSubmit, airNami)
      
    })
    this.restApiServices.waterSamplingCan(this.defaultFormSubmit).subscribe(data=>{
      let airCan = data;
      console.log('water Can',airCan);
      this._lineChartWaterCan(this.defaultFormSubmit, airCan)
      
    })
    this.restApiServices.waterSamplingPet(this.defaultFormSubmit).subscribe(data=>{
      let airPet = data;
  ...
  onSubmit(air:any,formData:any){
    ...
    if (air === 'air al4' && formData.micro != 'Pilih Micro' && formData.area != 'Area Sampling' && formData.date.from != undefined && formData.date.to != undefined) {
      this.restApiServices.waterSamplingAl4(submit).subscribe(data=>{
        let airAl4 = data;
        console.log('water al4',airAl4);
      ...
      this.restApiServices.waterSamplingNami(submit).subscribe(data=>{
        let airNami = data;
        console.log('water nami',airNami);
      ...
      this.restApiServices.waterSamplingCan(submit).subscribe(data=>{
        let airCan = data;
        console.log('water can',airCan);
      ...
      this.restApiServices.waterSamplingPet(submit).subscribe(data=>{
        let airPet = data;
        console.log('water pet',airPet);
